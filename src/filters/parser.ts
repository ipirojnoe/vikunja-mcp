...
export function parseSimpleFilter(filterStr: string): SimpleFilter | null {
  ...
  const match = filterStr.match(
    /^(\w+)\s*(not\s+in|like|in|!=|>=|<=|=|>|<)\s*(.+)$/i,
  );
  if (!match?.[1] || !match[2] || !match[3] || !SIMPLE_FILTER_FIELDS.has(match[1])) {
    return null;
  }

  const operator = match[2].toLowerCase().replace(/\s+/g, ' ') as SimpleFilter['operator'];
  const value = parseSimpleValue(match[3]);
  if (value === INVALID_SIMPLE_VALUE) {
    return null;
  }
...
const INVALID_SIMPLE_VALUE = Symbol('invalid-simple-filter-value');
...
function parseSimpleValue(value: string): unknown {
  ...
  if (trimmed.startsWith('[') || trimmed.startsWith('{')) {
    try {
      ...
      if (!Array.isArray(parsed) || parsed.length > 100) {
        return INVALID_SIMPLE_VALUE;
      }
      if (parsed.some((item) => !isSafeSimpleArrayValue(item))) {
        return INVALID_SIMPLE_VALUE;
      }
      return parsed;
    } catch {
  ...
  try {
    const parsed: unknown = JSON.parse(trimmed);
    return typeof parsed === 'object' && parsed !== null ? INVALID_SIMPLE_VALUE : parsed;
  } catch {
    return trimmed;
  }
}

function isSafeSimpleArrayValue(value: unknown): boolean {
  ...
  if (typeof value === 'string') {
    return value.length <= 100 &&
      !/[{};]|__proto__|prototype|constructor|function|eval|script|=>/i.test(value);
  }
  return false;
}
...
export function applyClientSideFilter(tasks: Task[], filter: SimpleFilter | null): Task[] {
  ...
  return tasks.filter((task) => compareSimpleValues(
    getSimpleTaskValue(task, filter.field),
    filter.operator,
    filter.value,
  ));
}

function getSimpleTaskValue(task: Task, field: string): unknown {
  const aliases: Record<string, string> = {
    dueDate: 'due_date',
    projectId: 'project_id',
  };
  const resolvedField = aliases[field] ?? field;

  if (resolvedField === 'labels') {
    return task.labels?.map((label) => typeof label === 'number' ? label : label.id)
      .filter((id): id is number => id !== undefined) ?? [];
  }
  if (resolvedField === 'assignees') {
    return task.assignees?.map((assignee) => typeof assignee === 'number' ? assignee : assignee.id) ?? [];
  }

  return (task as Record<string, unknown>)[resolvedField];
}

function compareSimpleValues(actual: unknown, operator: SimpleFilter['operator'], expected: unknown): boolean {
  if (operator === 'in' || operator === 'not in') {
    const expectedValues = Array.isArray(expected) ? expected : [expected];
    const actualValues = Array.isArray(actual) ? actual : [actual];
    const matches = expectedValues.some((value) => actualValues.includes(value));
  ...
  if (
    ...
    return compareOrdered(actualTime, operator, expectedTime);
  }

  return compareOrdered(actual, operator, expected);
}

function compareOrdered(actual: unknown, operator: SimpleFilter['operator'], expected: unknown): boolean {
  switch (operator) {
